<template>
  <div class="activity">
    <div class="activity_container">
      <el-row :gutter="20" style="min-height: 600px;margin-bottom:10px;">
          ...
          <div class="right-container">
            ...
            <div class="info">
              ...
              <div class="info-item">
                ...
                <p class="value" v-if="activityDetail.minLimitAmout > 0 && activityDetail.maxLimitAmout == 0"> ≥  {{fixedScale(activityDetail.minLimitAmout, activityDetail.amountScale)}}</p>
                <p class="value" v-if="activityDetail.minLimitAmout == 0 && activityDetail.maxLimitAmout > 0"> ≤  {{fixedScale(activityDetail.maxLimitAmout, activityDetail.amountScale)}}</p>
            ...
            <div class="bottom">
              <el-button type="warning" size="large" @click="apply">{{$t('activity.buyNow')}}</el-button>
            </div>
    ...
    <!-- modal -->
    <el-dialog v-model="modal2" width="360px" :close-on-click-modal="false">
      <template #title>
      ...
      <div style="text-align:center">
        <el-form ref="formValidateAddrRef" :model="formValidateAddr" :rules="ruleValidate" label-width="85px">
          <!-- 手机号 -->
      ...
      <template #footer>
        <el-button type="primary" size="large" @click="handleSubmit('formValidateAddrRef')">{{$t('activity.submit')}}</el-button>
      </template>
    </el-dialog>
...
const queryId = ref(0)
const formValidateAddrRef = ref(null)
...
const getData = () => {
  ...
  axios.post(`${host}/uc/mining/detail`, param, {
    ...
    if (res.data.code === 0) {
      ...
      document.title = (lang.value === '简体中文' ? '矿机 - ' : 'Mining - ') + activityDetail.detail + ' - MSCOIN | 全球比特币交易平台 | 全球数字货币交易平台'
      if (isLogin.value) {
        getWallet()
        getMember()
      }
...
const generateQRCode = async () => {
  const canvas = formValidateAddrRef.value?.$el?.querySelector('canvas') || document.querySelector('.show-qrcode canvas')
  if (canvas) {
    const qrValue = `${host}/mining/detail/${queryId.value}`
...
const attendClick = () => {
  ...
  attendAmount.value = 1
  modal2.value = true
}

const handleSubmit = (formRef) => {
  formValidateAddrRef.value.validate(valid => {
    if (valid) {
      submit(formRef)
    } else {
...
const submit = (name) => {
  ...
  axios.post(`${host}/uc/approve/buymining`, param, {
    withCredentials: true,
    headers: {
      'Content-Type': 'application/json',
      'x-auth-token': localStorage.getItem('TOKEN')
...
const getWallet = () => {
  axios.post(`${host}/uc/wallet${activityDetail.acceptUnit}`, {}, {
    ...
    if (res.data.code === 0) {
      mybalance.value = res.data.data.balance || 0
    }
  }).catch(() => {})
}
...
const getMember = () => {
  axios.post(`${host}/uc/approve/security/setting`, {}, {
    ...
    if (res.data.code === 0) {
      if (res.data.data.mobilePhone) {
        formValidateAddr.mobileNo = res.data.data.mobilePhone
        validPhone.value = true
        validEmail.value = false
      } else {
        formValidateAddr.email = res.data.data.email
...
onMounted(() => {
  ...
  qrcode.value = `${host}/mining/detail/${queryId.value}`
  init()
  getData()
})
</script>
